...
/**
 * FinanceSummary component for Admin.
 * Displays key financial metrics including Fundo Garantidor.
 * Per guardrails: All values are provided by backend, no calculations.
 */
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    ...
    <UCard>
      <div class="flex items-center gap-4">
        <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-success-100 dark:bg-success-900/30">
          <UIcon name="i-lucide-trending-up" class="w-6 h-6 text-success-600 dark:text-success-400" />
        </div>
        <div>
          ...
          <p class="text-xl font-semibold text-success-600 dark:text-success-400">
            {{ formatCurrency(summary.totalInflowCents) }}
          </p>
        </div>
      </div>
    </UCard>
    ...
    <UCard>
      <div class="flex items-center gap-4">
        <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-error-100 dark:bg-error-900/30">
          <UIcon name="i-lucide-trending-down" class="w-6 h-6 text-error-600 dark:text-error-400" />
        </div>
        <div>
          <p class="text-sm text-gray-500 dark:text-gray-400">Total Saídas</p>
          <p class="text-xl font-semibold text-error-600 dark:text-error-400">
            {{ formatCurrency(summary.totalOutflowCents) }}
          </p>
        </div>
      </div>
    </UCard>
    ...
    <UCard>
      <div class="flex items-center gap-4">
        ...
        <div>
          <p class="text-sm text-gray-500 dark:text-gray-400">Saldo Líquido</p>
          <p class="text-xl font-semibold text-gray-900 dark:text-white">
            {{ formatCurrency(summary.netBalanceCents) }}
          </p>
        </div>
      </div>
    </UCard>
